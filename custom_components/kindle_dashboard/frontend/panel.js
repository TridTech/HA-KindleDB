...
class KindleDashboardPanel extends HTMLElement {
  constructor() {
    ...
    this._hass     = null;
    this._config   = null;  // single source of truth — always a plain JS object
    this._entities = [];
  }
  ...
  async _loadEntities() {
    try {
      const res = await this._hass.callWS({ type: "kindle_dashboard/get_entities", domains: ALL_DOMAINS });
      this._entities = res.entities || [];
    } catch(e) { this._entities = []; }
  }

  // ── MOUNT — build static chrome + wire listeners once ──────────────────

  _mount() {
    const cfg = this._config || {};
    const font        = cfg.font || "Georgia, serif";
    const inlineUnits = !!cfg.inline_units;
    ...
    this.shadowRoot.innerHTML = `<style>${this._css()}</style>
      ...
      <div class="content">
        ...
        <div class="card">
          ...
          <div class="card-body">
            <p class="hint">Generate a Long-Lived Access Token in your HA profile, paste it below, then bookmark the resulting URL on your Kindle.</p>
            <div class="form-row">
            ...
            <div class="form-row">
              ...
              <div class="kindle-url" id="token-url-display">
                ${tokenUrl
          ...
          <div class="card-body">
            <div class="form-row">
              ...
              <input type="text" id="location-name"
                value="${this._esc(cfg.location_name || "Home")}" placeholder="Home">
            </div>
            <div class="form-row two-col">
              <div>
                ...
                <select id="font-select">
                  ${FONTS.map(f =>
                    `<option value="${this._esc(f.value)}"${f.value===font?" selected":""}>${f.label}</option>`
                  ).join("")}
                </select>
              </div>
              <div>
                ...
                <div class="row-inline" style="margin-top:6px">
                  ...
                  <span id="inline-units-label">
                    ${inlineUnits ? "Inline (22° C)" : "Stacked (value / unit)"}
                  </span>
                </div>
              </div>
            </div>
  ...
  // ── PAINT SECTIONS — pure render from this._config, no DOM reads ────────

  _paintSections() {
    const sections = (this._config || {}).sections || [];
    this.shadowRoot.querySelector("#sections-root").innerHTML =
      sections.map((sec, si) => this._sectionHTML(sec, si)).join("");
  }
  ...
  _sectionHTML(sec, si) {
    ...
    return `<div class="section-card">
      <div class="sec-header">
        ...
        <input class="sec-name" type="text" placeholder="Section name"
               value="${this._esc(sec.label||"")}">
        ${sec.type === "toggles"
          ? `<label class="two-col-wrap" title="Two-column layout">
               <input type="checkbox" class="sec-twocol"${twoCol?" checked":""}> 2-col
             </label>`
          : ""}
        <div class="sec-actions">
          <button class="btn-move-sec" data-dir="-1" title="Move up">↑</button>
          <button class="btn-move-sec" data-dir="1"  title="Move down">↓</button>
          <button class="btn-del-sec"  title="Remove section">🗑</button>
        </div>
      </div>
  ...
  _itemHTML(sectype, item, si, ii) {
    ...
    if (sectype === "sensors") {
      ...
      return `<div class="item-row" data-ii="${ii}">
        <select class="i-entity"><option value="">— sensor —</option>${opts}</select>
        <input class="i-label" type="text" placeholder="Label" value="${this._esc(item.label||"")}">
        <input class="i-unit"  type="text" placeholder="Unit"  value="${this._esc(item.unit||"")}" style="width:54px">
        ${movebtns}
    ...
    if (sectype === "scenes") {
      const opts = this._entities.filter(e => e.domain === "scene")
        .map(e => `<option value="${e.entity_id}"${e.entity_id===(item.entity||item.id)?" selected":""}>${e.name} (${e.entity_id})</option>`)
        .join("");
      return `<div class="item-row" data-ii="${ii}">
        <input class="i-icon"   type="text" placeholder="🎭" value="${this._esc(item.icon||"")}" style="width:40px;text-align:center">
        <input class="i-label"  type="text" placeholder="Name" value="${this._esc(item.name||"")}">
        <select class="i-entity"><option value="">— scene —</option>${opts}</select>
        ${movebtns}
        <button class="btn-del-item">✕</button>
      </div>`;
    }
    ...
    return `<div class="item-row" data-ii="${ii}">
      <input class="i-icon"  type="text" placeholder="💡" value="${this._esc(item.icon||"")}" style="width:40px;text-align:center">
      <input class="i-label" type="text" placeholder="Label" value="${this._esc(item.label||"")}">
      <select class="i-entity"><option value="">— entity —</option>${opts}</select>
  ...
  // ── LISTENERS — wired once, use closest() to find section/item indices ──
  ...
  _wireListeners() {
    ...
    root.addEventListener("change", (e) => {
      this._markDirty();
      if (e.target.id === "font-select") this._config.font = e.target.value;
      if (e.target.id === "inline-units") {
        this._config.inline_units = e.target.checked;
        root.querySelector("#inline-units-label").textContent =
          e.target.checked ? "Inline (22° C)" : "Stacked (value / unit)";
      }
    });
    ...
    root.addEventListener("click", (e) => {
      ...
      // Section-level buttons — find which section card we're in
      const card = btn.closest(".section-card");
      if (!card) return;
      const si = [...root.querySelectorAll(".section-card")].indexOf(card);
      if (si === -1) return;
      ...
      if (btn.classList.contains("btn-move-sec")) {
        this._doMoveSection(si, parseInt(btn.dataset.dir)); return;
      }

      // Item-level buttons — find which item row we're in
  ...
  // ── COLLECT current DOM state into a plain config object ────────────────
  // Uses DOM order (indexOf) not stale data-* indices.
  ...
  _collectConfig() {
    ...
    cfg.sections = [...root.querySelectorAll(".section-card")].map(card => {
      const sectype   = card.querySelector(".sec-badge")?.textContent || "toggles";
      ...
      const items = [...card.querySelectorAll(".item-row")].map(row => {
        const itype  = row.dataset.itemtype || sectype.replace(/s$/,"");
  ...
  // ── MUTATIONS — collect → mutate cfg → store → repaint ─────────────────
  ...
  _doAddItem(si) {
    ...
    if (!sec) return;
    if (sec.type === "sensors") sec.items.push({ id: "", label: "", unit: "", hide_from_status: false });
    else if (sec.type === "scenes") sec.items.push({ icon: "🎭", name: "", desc: "", entity: "" });
  ...
  _css() { return `
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    .row-inline{display:flex;align-items:center;gap:8px;font-size:13px}
    ...
    .ha-slider{position:absolute;cursor:pointer;inset:0;background:#ccc;border-radius:12px;transition:.2s}
    .ha-slider::before{content:"";position:absolute;width:18px;height:18px;left:3px;bottom:3px;
    ...
    .two-col-wrap{display:flex;align-items:center;gap:4px;font-size:12px;
      ...
      color:var(--secondary-text-color,#666);text-transform:none;letter-spacing:0;
      font-weight:400;margin-bottom:0}
    .two-col-wrap input{width:14px;height:14px;margin:0}
    ...
    .btn-move-item,.btn-del-item{background:none;border:none;cursor:pointer;
      padding:2px 4px;border-radius:3px;font-size:13px;flex-shrink:0;
      color:var(--secondary-text-color,#888)}
    .btn-del-item{color:var(--error-color,#db4437);font-size:15px}
  ...
  _toast(msg) {
    ...
    this._tt = setTimeout(() => { el.style.display = "none"; }, 2200);
  }
}
